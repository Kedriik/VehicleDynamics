...
std::size_t Delaunator::add_triangle(
	...
	//if(this->isTriangleValid(i0,i1,i2)){
...
bool Delaunator::isTriangleValid(std::size_t i0, std::size_t i1, std::size_t i2) {
	if (std::sqrt(std::pow(this->coords[i0]- this->coords[i1],2)+(std::pow(this->coords[i0+1] - this->coords[i1+1], 2)))>10) {
		return false;
	}
	if (std::sqrt(std::pow(this->coords[i1] - this->coords[i2], 2) + (std::pow(this->coords[i1 + 1] - this->coords[i2 + 1], 2))) > 10) {
		return false;
	}
	if (std::sqrt(std::pow(this->coords[i0] - this->coords[i2], 2) + (std::pow(this->coords[i0 + 1] - this->coords[i2 + 1], 2))) > 10) {
		return false;
	}
	return true;
}
