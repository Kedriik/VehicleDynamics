...
class ShapesGenerator {
	template <typename T>
	static void generateShape(std::vector<T>& vertices, std::vector<T>& normals, std::vector<T>& texCoords,int radius=1,int stackCount=100, int sectorCount=100) {
		double PI = 3.14159265359;
		// clear memory of prev arrays
		std::vector<T>().swap(vertices);
		std::vector<T>().swap(normals);
		std::vector<T>().swap(texCoords);
